// generated from rosidl_generator_c/resource/idl.h.em
// with input from iot_project_interfaces:msg/TargetsTimeLeft.idl
// generated code does not contain a copyright notice

#ifndef IOT_PROJECT_INTERFACES__MSG__TARGETS_TIME_LEFT_H_
#define IOT_PROJECT_INTERFACES__MSG__TARGETS_TIME_LEFT_H_

#include "iot_project_interfaces/msg/detail/targets_time_left__struct.h"
#include "iot_project_interfaces/msg/detail/targets_time_left__functions.h"
#include "iot_project_interfaces/msg/detail/targets_time_left__type_support.h"

#endif  // IOT_PROJECT_INTERFACES__MSG__TARGETS_TIME_LEFT_H_
