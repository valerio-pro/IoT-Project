...
def clustering(no_drones: int, targets: list, position: list, n_init: int = 10) -> dict:
...
def tsp(drone_assignment: dict, no_drones: int, position: list) -> dict:
    # "best_state" is the computed list of target indexes to be inspected
    for drone_id in range(no_drones):
        fitness_coords = mlrose.TravellingSales(coords=[(drone_target.x, drone_target.y, drone_target.z) for drone_target in drone_assignment[drone_id]])
        problem_fit = mlrose.TSPOpt(length=len(drone_assignment[drone_id]), fitness_fn=fitness_coords, maximize=False)
        best_state, _ = mlrose.genetic_alg(problem_fit, mutation_prob=0.2, max_attempts=10)
        old_assignment: list = drone_assignment[drone_id]
...
def trivial_case(position: list, targets: list, no_drones: int) -> dict:
    available_drones: dict = {d: position[d] for d in range(no_drones)}
    drone_assignment: dict = {}
...
# Executed when no_drones >= len(targets)
def trivial_case_2(targets: list, no_drones: int) -> dict:
    drone_assignment: dict = {}
    curr_drone_id: int = 0
    for target in targets:
...
def assign_drones_to_clusters(position: list, centroids_targets_assignment: dict, no_drones: int) -> dict:
    available_drones: dict = {d: position[d] for d in range(no_drones)}
    drone_assignment: dict = {}
...
def compute_closest_drone_to_target(target: Point, available_drones: dict) -> int:
...
def compute_closest_drone_to_centroid(centroid: tuple, available_drones: dict) -> int:
    best_distance = inf
    best_drone = None
    for drone, drone_position in available_drones.items():
        drone_tuple: tuple = (drone_position.x, drone_position.y, drone_position.z)
...
def compute_closest_target_to_drone(drone_position: Point, targets: list) -> tuple:
    best_distance = inf
    best_target = None
    best_idx = None
...
def all_positions_initialized(position: list) -> bool:
    for pos in position:
        if not pos:
            return False
    return True
